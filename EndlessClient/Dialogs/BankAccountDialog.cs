...
namespace EndlessClient.Dialogs
{
    public class BankAccountDialog : ScrollingListDialog
    {
            ...
            _characterInventoryProvider.ItemInventory.SingleOrNone(x => x.ItemID == 1)
                .Match(
                    some: characterGold =>
                    {
                        ...
                        {
                            var dlg = _itemTransferDialogFactory.CreateItemTransferDialog(_eifFileProvider.EIFFile[1].Name, ItemTransferType.BankTransfer, characterGold.Amount, EOResourceID.DIALOG_TRANSFER_DEPOSIT);
        ...
        private void Withdraw(object sender, EventArgs e)
        {
            ...
            {
                var dlg = _itemTransferDialogFactory.CreateItemTransferDialog(_eifFileProvider.EIFFile[1].Name, ItemTransferType.BankTransfer, _bankDataProvider.AccountValue, EOResourceID.DIALOG_TRANSFER_WITHDRAW);
                dlg.DialogClosing += (_, e) =>
                {
                    if (e.Result == XNADialogResult.OK)
